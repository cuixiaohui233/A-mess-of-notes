    ...
    for rela in relations:
        ...
        for r in rela[1]:
            if len(rela[1]) == 3:
                ...
                relation1 = '<relation id="'+str(rela[0])+'" version="4" visible="true">\n<member ref="'+rela[1][0][1:-1].split(',')[1]+'" role="from" type="way"/>\n<member ref="'+rela[1][2][1:-1].split(',')[1]+'" role="via" type="way"/>\n<member ref="'+rela[1][3][1:-1].split(',')[1]+'" role="to" type="way"/>\n'
                deg = int(rela[1][2][1:-1].split(',')[2]) + int(rela[1][3][1:-1].split(',')[2]) - 180
                if deg < 45 or deg >= 315:
                    relation1 += '<tag k="restriction" v="no_u_turn"/>\n'
                elif deg >= 45 or deg < 135:
                    relation1 += '<tag k="restriction" v="no_left_turn"/>\n'
                elif deg >= 135 or deg <= 225:
                    relation1 += '<tag k="restriction" v="no_straight_on"/>\n'
                elif deg >= 225 or deg < 315:
                    relation1 += '<tag k="restriction" v="no_right_on"/>\n'
                relation1 += '<tag k="type" v="restriction"/>\n</relation>\n'
                fo.write(relation1)
